...
%% --- Select which analytes to display ---1
...
%% --- Plot 1: Average Concentration with Standard Deviation Error Bars ---
figure;
hold on;
nAnalytes = size(selectedAvgConcentration, 2);
hStd = gobjects(1, nAnalytes);
for i = 1:nAnalytes
    hStd(i) = errorbar(xValues, selectedAvgConcentration(:, i), selectedStdDev(:, i), ...
        'LineStyle', '-', 'Marker', 'o', 'MarkerSize', 8, 'LineWidth', 1.5);
end
hold off;

% Customize font sizes
xlabel('Digestion acid concentration', 'FontSize', 24);
ylabel('Concentration [wt. ppm]', 'FontSize', 24);
title('Average Concentration with Standard Deviation Error Bars', 'FontSize', 18);
legend(hStd, analyteNames, 'Location', 'Best', 'FontSize', 20);

grid on;
box on;

% Set the custom x-axis tick labels, limits, and font size
set(gca, 'XTick', xValues, 'XTickLabel', customXLabels, 'FontSize', 22);
xlim([min(xValues)-padding, max(xValues)+padding]);

%% --- Plot 2: Average Concentration with Propagated Error Bars ---
figure;
hold on;
hProp = gobjects(1, nAnalytes);
for i = 1:nAnalytes
    hProp(i) = errorbar(xValues, selectedAvgConcentration(:, i), selectedPropError(:, i), ...
        'LineStyle', '-', 'Marker', 'o', 'MarkerSize', 8, 'LineWidth', 1.5);
end
hold off;
...
legend(hProp, analyteNames, 'Location', 'Best', 'FontSize', 20);
